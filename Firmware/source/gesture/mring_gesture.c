...
void smooth_short_ACC_period_data(short* period_data, int smooth_span, int* period_data_smooth)
{
...
void smooth_ACC_period_data(uint16_t* period_data, int smooth_span, int* period_data_smooth)
{
...
void mean_get_int(int *data,int dataLen,int* meanValue)
{   
...
int var_get_int(int* data, int dataLen)
{
    if(data == NULL || dataLen <= 1){
        return 0;
    }
	...
	{
		//all_data_sum = all_data_sum + pow(data[num]- mean_value,2);
		all_data_sum = all_data_sum + (data[num]- mean_value)*(data[num]- mean_value);
	}
	var_value = all_data_sum / (dataLen - 1);
	return var_value;
}
...
void max_get_int(int* data,int dataLen,int* maxValue,int* maxValuePos)
{
	*maxValue = data[0];
	*maxValuePos = 0;
	for (int num = 0;num < dataLen;num++)
...
void min_get_int(int* data, int dataLen, int* minValue, int* minValuePos)
{
	*minValue = data[0];
	*minValuePos = 0;
	for (int num = 0;num < dataLen;num++)
...
void cross_mean_cnt_get(int* data, int dataLen, int meanValue,int *cross_mean_cnt)
{
...
PERIOD_PEAK_POS g_prePeriod = {0};
PERIOD_PEAK_POS g_nowPeriod = {0};
void Count_Move_get_new(uint16_t* input, int inlen, int* period_step_count)
{
	int period_data_smooth[STEP_COUNT_PERIOD] = {0};
	int mean_value = 0;
	int mean_pos_DownToUp[STEP_COUNT_PERIOD / 2] = {0};
	int mean_pos_UpToDown[STEP_COUNT_PERIOD / 2] = {0};
	...
	int maxPeakValueMax = 0 ;
	int maxPeakValueMaxPos = 0;
	int minPeakValueMin = 0 ;
	int minPeakValueMinPos = 0;
	int secondMaxPeakValueMax = 0;
	uint8_t UpToDownEndPointFlag = 0;
	uint8_t DownToUpEndPointFlag = 0;
	uint16_t peak_num = 0;
	smooth_ACC_period_data(input, SMOOTH_SPAN, period_data_smooth);

	mean_get_int(period_data_smooth, STEP_COUNT_PERIOD, &mean_value);
	mean_value = mean_value - 100;
	//printf("mean_value: %d\n", mean_value);
	for (int num_1 = 0; num_1 < STEP_COUNT_PERIOD - 1; num_1++)
	{
		if (mean_pos_DownToUp_cnt == 0)
		{
		...
		//else if (period_data_smooth[num_1] < mean_value && period_data_smooth[num_1 + 1] >= mean_value && abs(num_1 - mean_pos_DownToUp[mean_pos_DownToUp_cnt - 1]) > ACC_SAMPLERATE / 5)
		else if (period_data_smooth[num_1] < mean_value && period_data_smooth[num_1 + 1] >= mean_value)
		{
			mean_pos_DownToUp[mean_pos_DownToUp_cnt] = num_1;
			mean_pos_DownToUp_cnt++;
		}
		if(mean_pos_UpToDown_cnt == 0)
		{
			if (period_data_smooth[num_1] >= mean_value && period_data_smooth[num_1 + 1] < mean_value )
			{
		...
		//else if (period_data_smooth[num_1] >= mean_value && period_data_smooth[num_1 + 1] < mean_value && abs(num_1 - mean_pos_UpToDown[mean_pos_UpToDown_cnt - 1]) > ACC_SAMPLERATE / 5)
		else if (period_data_smooth[num_1] >= mean_value && period_data_smooth[num_1 + 1] < mean_value)
		{
			mean_pos_UpToDown[mean_pos_UpToDown_cnt] = num_1;
			mean_pos_UpToDown_cnt++;
	...
	//UpToDown end point process
	UpToDownEndPointFlag = 0;
	for (int UpToDownNum = 0; UpToDownNum < mean_pos_UpToDown_cnt; UpToDownNum++)
	{
		if (mean_pos_DownToUp[mean_pos_DownToUp_cnt - 1] < mean_pos_UpToDown[UpToDownNum])
		{
			UpToDownEndPointFlag = 1;
		}
	}
	if (UpToDownEndPointFlag == 0)
	{
		mean_pos_UpToDown[mean_pos_UpToDown_cnt] = STEP_COUNT_PERIOD - 1;
		mean_pos_UpToDown_cnt++;
	}
	//DownToUp start point process
	DownToUpEndPointFlag = 0;
	for (int DownToUpNum = 0; DownToUpNum < mean_pos_DownToUp_cnt; DownToUpNum++)
	{
		if (mean_pos_UpToDown[0] > mean_pos_DownToUp[DownToUpNum])
		{
			DownToUpEndPointFlag = 1;
		}
	}
	if (DownToUpEndPointFlag == 0)
	{
		for (int num = mean_pos_DownToUp_cnt;num > 0;num--)
		{
			mean_pos_DownToUp[num] = mean_pos_DownToUp[num - 1];
		}
		mean_pos_DownToUp[0] = 0;
		mean_pos_DownToUp_cnt++;
	}



	g_nowPeriod.lastPos = STEP_COUNT_PERIOD;
	g_nowPeriod.firstPos = STEP_COUNT_PERIOD;
	for (int num_2 = 0;num_2 < mean_pos_DownToUp_cnt;num_2++)
	{
		//for (int num_3 = 0;num_3 < mean_pos_UpToDown_cnt;num_3++)
		for (int num_3 = num_2; num_3 < mean_pos_UpToDown_cnt; num_3++)
		{
			if (mean_pos_DownToUp[num_2] < mean_pos_UpToDown[num_3] && mean_pos_UpToDown[num_3] - mean_pos_DownToUp[num_2] > ACC_STEP_SAMPLERATE/MAX_STEP_PER_SECOND) //ACC_STEP_SAMPLERATE/5 is max stride frequency
			{
				...
				{
					if (num_4 > 3 && num_4 < STEP_COUNT_PERIOD - 3)
					{
					...
					if ((num_4 <= 3 && num_4 > 1) || (num_4 >= STEP_COUNT_PERIOD - 3 && num_4 < STEP_COUNT_PERIOD - 2))
					{
						if (period_data_smooth[num_4] >= period_data_smooth[num_4 - 1]  && period_data_smooth[num_4] >= period_data_smooth[num_4 - 2] && period_data_smooth[num_4] >= period_data_smooth[num_4 + 1] && period_data_smooth[num_4] >= period_data_smooth[num_4 + 2])
						{
							period_max_peak.maxValue[period_max_peak.maxValueCnt] = period_data_smooth[num_4];
							period_max_peak.maxValuePos[period_max_peak.maxValueCnt] = num_4;
							period_max_peak.maxValueCnt++;
				...
				{
					max_get_int(period_max_peak.maxValue, period_max_peak.maxValueCnt,&maxPeakValueMax,&maxPeakValueMaxPos);
					min_get_int(period_min_peak.minValue, period_min_peak.minValueCnt,&minPeakValueMin,&minPeakValueMinPos);
					...
					if (maxPeakValueMax - minPeakValueMin >= STEP_ACC_CHANGE_THRESHOLD && secondMaxPeakValueMax - minPeakValueMin >= STEP_ACC_CHANGE_THRESHOLD_TWO && (maxPeakValueMax > STEP_ACC_PEAK_MIN_THRESHOLD && secondMaxPeakValueMax > STEP_ACC_PEAK_MIN_THRESHOLD))
					{
						*period_step_count = *period_step_count + 2;
						peak_num = peak_num + 1;
						g_nowPeriod.lastPos = STEP_COUNT_PERIOD -  period_max_peak.maxValuePos[maxPeakValueMaxPos];
					}
					else if (maxPeakValueMax - minPeakValueMin >= STEP_ACC_CHANGE_THRESHOLD && secondMaxPeakValueMax - minPeakValueMin >= STEP_ACC_CHANGE_THRESHOLD_TWO && (maxPeakValueMax > STEP_ACC_PEAK_MIN_THRESHOLD || secondMaxPeakValueMax > STEP_ACC_PEAK_MIN_THRESHOLD))
					{
					...
					else if(maxPeakValueMax > STEP_ACC_PEAK_MIN_THRESHOLD)
					{
						*period_step_count = *period_step_count + 1;
						peak_num = peak_num + 1;
						g_nowPeriod.lastPos = STEP_COUNT_PERIOD -  period_max_peak.maxValuePos[maxPeakValueMaxPos];
				...
				else if(period_max_peak.maxValue[0] > STEP_ACC_PEAK_MIN_THRESHOLD)
				{
					*period_step_count = *period_step_count + 1;
					peak_num = peak_num + 1;
					g_nowPeriod.lastPos = STEP_COUNT_PERIOD -  period_max_peak.maxValuePos[0];
				}


				if(peak_num == 1)
				{
					g_nowPeriod.firstPos = period_max_peak.maxValuePos[maxPeakValueMaxPos];
				}
				//break;
			}
			break;
	...
	if(g_nowPeriod.firstPos + g_prePeriod.lastPos > STEP_COUNT_PERIOD)
	{
		*period_step_count = 0;
	}
	memcpy(&g_prePeriod,&g_nowPeriod,sizeof(PERIOD_PEAK_POS));
...
void Count_Move_get_new_walk(uint16_t* input, int inlen, int* period_step_count)
{
	int period_data_smooth[STEP_COUNT_PERIOD] = { 0 };
	int mean_value = 0;
	int first_mean_value = 0;
	int mean_pos_DownToUp[STEP_COUNT_PERIOD / 2] = { 0 };
	int mean_pos_UpToDown[STEP_COUNT_PERIOD / 2] = { 0 };
	int mean_pos_DownToUp_cnt = 0;
	int mean_pos_UpToDown_cnt = 0;
	...
	mean_get_int(period_data_smooth, STEP_COUNT_PERIOD, &mean_value);
	first_mean_value = mean_value;
	int mean_value_new = 0;
	int dataNum = 0;
	for (int num = 0;num < STEP_COUNT_PERIOD;num++)
	{
		if (period_data_smooth[num] < mean_value)
		{
			mean_value_new = mean_value_new + period_data_smooth[num];
			dataNum = dataNum + 1;
		}
	}
	if (dataNum != 0)
	{
		mean_value_new = mean_value_new / dataNum;
		mean_value = mean_value_new;
	}
	// printf("2mean_value: %d\n", mean_value);
	mean_value_new = 0;
	dataNum = 0;
	for (int num = 0; num < STEP_COUNT_PERIOD; num++)
	{
	...
	for (int num = 0; num < STEP_COUNT_PERIOD; num++)
	{
		if (period_data_smooth[num] > first_mean_value)
		{
			mean_value_new = mean_value_new + period_data_smooth[num];
			dataNum = dataNum + 1;
		}
	...
	for (int num = 0; num < STEP_COUNT_PERIOD; num++)
	{
		if (period_data_smooth[num] > peaks_mean)
		{
			mean_value_new = mean_value_new + period_data_smooth[num];
			dataNum = dataNum + 1;
		}
	...
	// printf("2peaks_mean: %d\n", peaks_mean);


	int ampl_change = 0;
	ampl_change = peaks_mean - mean_value;
	// printf("ampl_change: %d\n", ampl_change);
	...
	for (int num_2 = 0; num_2 < mean_pos_DownToUp_cnt; num_2++)
	{
		...
		for (int num_3 = num_2; num_3 < mean_pos_UpToDown_cnt; num_3++)
		{
			...
			//if (mean_pos_DownToUp[num_2] < mean_pos_UpToDown[num_3] && mean_pos_UpToDown[num_3] - mean_pos_DownToUp[num_2] > ACC_STEP_SAMPLERATE / 5)
			if (mean_pos_DownToUp[num_2] < mean_pos_UpToDown[num_3] && mean_pos_UpToDown[num_3] - mean_pos_DownToUp[num_2] > ACC_STEP_SAMPLERATE / 10)
			{
				memset(&period_max_peak, 0, sizeof(MAX_PARA));
				memset(&period_min_peak, 0, sizeof(MIN_PARA));
				for (int num_4 = mean_pos_DownToUp[num_2]; num_4 < mean_pos_UpToDown[num_3]; num_4++)
...
#endif
				if (period_max_peak.maxValueCnt >= 1)
				{
					max_get_int(period_max_peak.maxValue, period_max_peak.maxValueCnt, &maxPeakValueMax, &maxPeakValueMaxPos);
					//min_get_int(period_min_peak.minValue, period_min_peak.minValueCnt, &minPeakValueMin, &minPeakValueMinPos);

					//if (maxPeakValueMax - mean_value >= ampl_change)
					if (maxPeakValueMax - mean_value >= ampl_change*9/10)
					{
						*period_step_count = *period_step_count + 2;
					}
...
int g_down_peaks_value = 0;
int g_maxPMinPFlag = 0;
void Count_Move_get_new_walk_new(uint16_t* input, int inlen, int* period_step_count)
{
	int period_data_smooth[STEP_COUNT_PERIOD] = { 0 };
	int mean_value = 0;
	int first_mean_value = 0;
	...
	int maxValue = period_data_smooth[0];
	int minValue = period_data_smooth[0];
	int max_min_diff = 0;
	int amp_max_min = 0;
	for (int num = 0;num < STEP_COUNT_PERIOD;num++)
	{
		if (maxValue < period_data_smooth[num])
		{
			maxValue = period_data_smooth[num];
		}
		if (minValue > period_data_smooth[num])
		{
			minValue = period_data_smooth[num];
		}
	}
	amp_max_min = maxValue - minValue;
	max_min_diff = minValue + amp_max_min / 2;
	// printf("amp_max_min = %d\n", amp_max_min);
	// printf("g_maxPMinPFlag = %d\n", g_maxPMinPFlag);
	// printf("g_down_peaks_value = %d\n", g_down_peaks_value);
	int downPeaksValue = 0;
	int upPeaksValue = 0;
	if (g_maxPMinPFlag == 1)
	{
		
		period_min_peak.minValue[period_min_peak.minValueCnt] = g_down_peaks_value;
		period_min_peak.minValuePos[period_min_peak.minValueCnt] = 1;
		period_min_peak.minValueCnt++;
	}
	for (int num = 4;num < STEP_COUNT_PERIOD-4;num++)
	{
		if (period_data_smooth[num] <= period_data_smooth[num - 1] && period_data_smooth[num] < period_data_smooth[num - 2] && period_data_smooth[num] < period_data_smooth[num - 3] && period_data_smooth[num] < period_data_smooth[num - 4] && period_data_smooth[num] <= period_data_smooth[num + 1] && period_data_smooth[num] < period_data_smooth[num + 2] && period_data_smooth[num] < period_data_smooth[num + 3] && period_data_smooth[num] < period_data_smooth[num + 4])
		{
			if (period_data_smooth[num] < max_min_diff)
			{
				period_min_peak.minValue[period_min_peak.minValueCnt] = period_data_smooth[num];
				period_min_peak.minValuePos[period_min_peak.minValueCnt] = num;
				period_min_peak.minValueCnt++;
		...
		if (period_data_smooth[num] >= period_data_smooth[num - 1] && period_data_smooth[num] > period_data_smooth[num - 2] && period_data_smooth[num] > period_data_smooth[num - 3] && period_data_smooth[num] > period_data_smooth[num - 4] && period_data_smooth[num] >= period_data_smooth[num + 1] && period_data_smooth[num] > period_data_smooth[num + 2] && period_data_smooth[num] > period_data_smooth[num + 3] && period_data_smooth[num] > period_data_smooth[num + 4])
		{
			period_max_peak.maxValue[period_max_peak.maxValueCnt] = period_data_smooth[num];
			period_max_peak.maxValuePos[period_max_peak.maxValueCnt] = num;
			period_max_peak.maxValueCnt++;
		}
	...
	for (int minNum = 0;minNum < period_min_peak.minValueCnt; )
	{
		
		if (period_min_peak.minValuePos[minNum] > preMaxP)
		{
			g_maxPMinPFlag = 1;
			maxPMinPdiff = 0;
			for (int maxNum = 0; maxNum < period_max_peak.maxValueCnt; maxNum++)
			{

				for (int num = minNum + 1;num < period_min_peak.minValueCnt;num++)
				{
					if (period_min_peak.minValuePos[num] > period_min_peak.minValuePos[minNum] && period_min_peak.minValuePos[num] < period_max_peak.maxValuePos[maxNum])
					{
						if (period_min_peak.minValuePos[num] < period_min_peak.minValuePos[minNum])
						{
							minNum = num;
						}
				...
				if (period_min_peak.minValuePos[minNum] < period_max_peak.maxValuePos[maxNum])
				{
					
					maxPMinPdiff = period_max_peak.maxValue[maxNum] - period_min_peak.minValue[minNum];
					//printf("maxPMinPdiff = %d\n", maxPMinPdiff);
					
					if (maxPMinPdiff > amp_max_min*0.7 && maxPMinPdiff > 500)
					{
						// printf("maxPMinPdiff = %d\n", maxPMinPdiff);
						g_maxPMinPFlag = 0;
						preMaxP = period_max_peak.maxValuePos[maxNum];
						*period_step_count = *period_step_count + 2;
						break;
					}
			...
			if (g_maxPMinPFlag == 1)
			{
				g_down_peaks_value = period_min_peak.minValue[minNum];
			}
		}
		minNum = minNum + 1;
	}
...
int g_three_step[PERIOD_NUM] = {0};

int g_vaild_step_flag[PERIOD_NUM] = {0};

int g_four_step[PERIOD_NUM_NEW] = {0};
int g_four_vaild_step_flag[PERIOD_NUM_NEW] = {0};

void updata_step_period_record(int *step_record,int period_len,int period_step)
{
	for (int num = 0;num < period_len - 1;num++)
	{
		step_record[num] = step_record[num+1];
		
	}
	step_record[period_len - 1] = period_step;

}
uint8_t g_keep_vaild_step_num = KEEP_VAILD_STEP_NUM;
uint8_t g_keep_period_step_num = KEEP_PERIOD_DAILY_STEP_NUM;
uint8_t g_keep_period_step_num_max = KEEP_PERIOD_DAILY_STEP_NUM_MAX;
void step_no_sport_delete(int *period_step)
{
	int step_sum = 0;
	updata_step_period_record(g_three_step, PERIOD_NUM, *period_step);
	for (int num = 0;num < PERIOD_NUM;num++)
	{
		step_sum = step_sum + g_three_step[num];
	}

	if(ACC_STEP_SAMPLERATE == 50)
	{
		g_keep_vaild_step_num = KEEP_VAILD_STEP_NUM;
	}
	else if(ACC_STEP_SAMPLERATE == 100)
	{
		g_keep_vaild_step_num = KEEP_EHR_VAILD_STEP_NUM;
	}
	// if (step_sum <= KEEP_VAILD_STEP_NUM)
	if (step_sum <= g_keep_vaild_step_num)
	{
		*period_step = 0;
		g_vaild_step_flag[PERIOD_NUM - 1] = STEP_PERIOD_NO_VAILD_FLAG;
		updata_step_period_record(g_vaild_step_flag, PERIOD_NUM, STEP_PERIOD_NO_VAILD_FLAG);
	}
	else
	{

		if (g_vaild_step_flag[1] == STEP_PERIOD_NO_VAILD_FLAG && g_vaild_step_flag[0] == STEP_PERIOD_NO_VAILD_FLAG)
		{
			*period_step = *period_step + g_three_step[0] + g_three_step[1];
		}
		else if (g_vaild_step_flag[1] == STEP_PERIOD_NO_VAILD_FLAG)
		{
			*period_step = *period_step + g_three_step[1];
		}
		g_vaild_step_flag[PERIOD_NUM - 1] = STEP_PERIOD_VAILD_FLAG;
		updata_step_period_record(g_vaild_step_flag, PERIOD_NUM, STEP_PERIOD_VAILD_FLAG);
	}
}


void step_no_sport_delete_new(int *period_step)
{
	int step_sum = 0;
	updata_step_period_record(g_four_step, PERIOD_NUM_NEW, *period_step);
	for (int num = 0;num < PERIOD_NUM_NEW;num++)
	{
		step_sum = step_sum + g_four_step[num];
	}

	if(ACC_STEP_SAMPLERATE == 50)
	{
		g_keep_vaild_step_num = KEEP_VAILD_STEP_NUM;
		g_keep_period_step_num = KEEP_PERIOD_DAILY_STEP_NUM;
		g_keep_period_step_num_max = KEEP_PERIOD_DAILY_STEP_NUM_MAX;
	}
	else if(ACC_STEP_SAMPLERATE == 100)
	{
		g_keep_vaild_step_num = KEEP_EHR_VAILD_STEP_NUM;
		g_keep_period_step_num = KEEP_PERIOD_SPORT_STEP_NUM;
		g_keep_period_step_num_max = KEEP_PERIOD_SPORT_STEP_NUM_MAX;
	}
	// if (step_sum <= KEEP_VAILD_STEP_NUM)
	if (step_sum <= g_keep_vaild_step_num)
	{
		*period_step = 0;
		g_four_vaild_step_flag[PERIOD_NUM_NEW - 1] = STEP_PERIOD_NO_VAILD_FLAG;
		updata_step_period_record(g_four_vaild_step_flag, PERIOD_NUM_NEW, STEP_PERIOD_NO_VAILD_FLAG);
	}
	else if(g_four_step[PERIOD_NUM_NEW - 1] > g_keep_period_step_num_max)
	{
		*period_step = 0;
		g_four_step[PERIOD_NUM_NEW - 1] = 0;
		g_four_vaild_step_flag[PERIOD_NUM_NEW - 1] = STEP_PERIOD_NO_VAILD_FLAG;
		updata_step_period_record(g_four_vaild_step_flag, PERIOD_NUM_NEW, STEP_PERIOD_VAILD_FLAG);
	}
	else if((g_four_step[PERIOD_NUM_NEW - 1] >= g_keep_period_step_num) && (g_four_step[PERIOD_NUM_NEW - 1] + g_four_step[PERIOD_NUM_NEW - 2] <= (g_four_step[PERIOD_NUM_NEW - 3] + g_four_step[PERIOD_NUM_NEW - 4])*1.5) && (g_four_step[PERIOD_NUM_NEW - 1] + g_four_step[PERIOD_NUM_NEW - 2] >= (g_four_step[PERIOD_NUM_NEW - 3] + g_four_step[PERIOD_NUM_NEW - 4])*0.5))
	{

		if (g_four_vaild_step_flag[PERIOD_NUM_NEW - 2] == STEP_PERIOD_NO_VAILD_FLAG && g_four_step[PERIOD_NUM_NEW - 2] >= g_keep_period_step_num && g_four_vaild_step_flag[PERIOD_NUM_NEW - 3] == STEP_PERIOD_NO_VAILD_FLAG && g_four_step[PERIOD_NUM_NEW - 3] >= g_keep_period_step_num)
		{
			*period_step = *period_step + g_four_step[PERIOD_NUM_NEW - 2] + g_four_step[PERIOD_NUM_NEW - 3];
		}
		// else if (g_four_vaild_step_flag[PERIOD_NUM_NEW - 2] == STEP_PERIOD_NO_VAILD_FLAG && g_four_step[PERIOD_NUM_NEW - 2] >= g_keep_period_step_num)
		// {
		// 	*period_step = *period_step + g_four_step[1];
		// }
		g_four_vaild_step_flag[PERIOD_NUM_NEW - 1] = STEP_PERIOD_VAILD_FLAG;
		updata_step_period_record(g_four_vaild_step_flag, PERIOD_NUM_NEW, STEP_PERIOD_VAILD_FLAG);
	}
	else{
		*period_step = 0;
...
// down add xuan
uint32_t period_step_count = 0;
int period_cnt = 0;
int accx_period_data_smooth[STEP_COUNT_PERIOD] = { 0 };
int accy_period_data_smooth[STEP_COUNT_PERIOD] = { 0 };
int accz_period_data_smooth[STEP_COUNT_PERIOD] = { 0 };
...
int accz_var = 0;
int g_stepACCgsum = 0;
uint32_t g_stepACCgCnt = 0;
extern uint8_t g_sleep_flag;
extern uint8_t g_sleep_start_check_flag;
// UTCTime start_t = 0;
// UTCTime now_t = 0;

int64_t sqrt_bv(int64_t n)
{
    int64_t sqrt = 0;
    int64_t shift = 15;
    int64_t sqrt2;
...
void stepsnum_update(acc_axis *pt, E_ACC_PROC statu)
{
    ...
    else
    {
		...
		//xyz = 9900;
		gdataRaw = (int)(sqrt_bv(xyz*100)/10);
		//NRF_LOG_INFO("========gdataRaw = %d\n",gdataRaw);
		// gdataRaw = pow(abs((int)ACC_x*ACC_x + (int)ACC_y*ACC_y + (int)ACC_z*ACC_z), 0.5);
		//gdataRaw = 20;
        gdata = (uint16_t)gdataRaw;
		gdataACC = (int)gdataRaw;
    }

	// if(g_stepACCgCnt == 0)
	// {
	// 	start_t = UTC_getClock();
	// 	NRF_LOG_INFO("1111111==============start_t = %d\n",start_t);
	// }

	if(g_sleep_flag == END_SLEEP_MODE)
	{
		g_stepACCgsum = g_stepACCgsum + gdataACC;
		g_stepACCgCnt = g_stepACCgCnt + 1;
		//NRF_LOG_INFO("==============g_stepACCgCnt = %d\n",g_stepACCgCnt);
		//auto sleep ACC
		int ACCgmean = 0;
		// now_t = UTC_getClock();
		// NRF_LOG_INFO("==============start_t = %d\n",start_t);
		// NRF_LOG_INFO("==============now_t = %d\n",now_t);
		// if(now_t - start_t >= 1)// 50Hz 
		if(g_stepACCgCnt >= 50)// step ACC Sample 50Hz
		{
			ACCgmean = g_stepACCgsum/g_stepACCgCnt;
			g_stepACCgCnt = 0;
			g_stepACCgsum = 0;
			int tmpaccval = abs(gdataACC - (int)ACCgmean);
			uint8_t ehr_flg = 0;
			...
			step_accval = data_compress16(tmpaccval,ehr_flg,0);

			if(acc_get_Sleep_flag() == END_SLEEP_MODE)
			{
				Sleep_start_judge_acc_get(step_accval & 0x7F);
			}
		
			//NRF_LOG_INFO("123==============g_sleep_start_check_flag = %d\n",g_sleep_start_check_flag);

			//Exercise_judge_acc_get(accval);
			//NRF_LOG_INFO("123==============accval = %d\n",accval);
			//NRF_LOG_INFO("123==========acc_get_Exercise_flag(void) = %d\n",acc_get_Exercise_flag());
		}
	}
	else{
		//NRF_LOG_INFO("=================================\n");
...
//put data in buf
        ...
        if (fifo16_status(&acc_g_fifo) >= step_gbuf_length)
        {
			...
			if (statu == e_acc_EHR)
            {
				...
				// Count_Move_get_new_walk(acc_g_fifo.buf, step_gbuf_length, &period_step_count);
				//Count_Move_get_new_walk_new(acc_g_fifo.buf, step_gbuf_length, &period_step_count);
            }
			...
			memset(accx_period_data_smooth,0, STEP_COUNT_PERIOD * sizeof(int));
			memset(accy_period_data_smooth, 0, STEP_COUNT_PERIOD * sizeof(int));
			memset(accz_period_data_smooth, 0, STEP_COUNT_PERIOD * sizeof(int));
			...
			// mean_accy = 0;
			// mean_accz = 0;
			// mean_get_int(accx_period_data_smooth, STEP_COUNT_PERIOD,&mean_accx);
			// mean_get_int(accy_period_data_smooth, STEP_COUNT_PERIOD, &mean_accy);
			// mean_get_int(accz_period_data_smooth, STEP_COUNT_PERIOD, &mean_accz);
			...
			accy_var = 0;
			accz_var = 0;
			accx_var = var_get_int(accx_period_data_smooth, STEP_COUNT_PERIOD);
			accy_var = var_get_int(accy_period_data_smooth, STEP_COUNT_PERIOD);
			accz_var = var_get_int(accz_period_data_smooth, STEP_COUNT_PERIOD);
			...
			// step_no_sport_delete(&period_step_count);
			step_no_sport_delete_new(&period_step_count);
			...
			// down add xuan
			fifo16_recover(&acc_g_fifo, 0);
			// up add xuan
        	Exercise_judge_acc_get(acc_para.stdaccval);
			
			//NRF_LOG_INFO("1111==========acc_para.stdaccval = %d\n",acc_para.stdaccval);
			//NRF_LOG_INFO("1111======acc_get_Exercise_flag(void) = %d\n",acc_get_Exercise_flag());
...
//short g_sport_acc[AAC_RECORD_MAX_NUM] = {0};
//uint16_t g_sport_acc_cnt = 0;

static float ACCgsum = 0;

int AccDataRealtime[ACC_13_SAMPLERATE] = {0};
void EHR_Acc_Proc(acc_axis*pt)
{
	...
	AccDataRealtime[accdata.accstfp%length_accval] = accdata.ACC_g;
...
//        acc_3axis_log_put((uint16_t)accdata.ACC_g);
		...
		//if(g_sport_acc_cnt >= AAC_RECORD_MAX_NUM)
		//{
		//	g_sport_acc_cnt = 0;
		//}
		//g_sport_acc[g_sport_acc_cnt] = (short)accdata.ACC_g;
		//g_sport_acc_cnt = g_sport_acc_cnt + 1;
    }
    else
    {
       ...
       // tmp_acc_outbuf[6] = EHR_acc_filter(tmp_acc_inbuf, coffa, coffb, cofflen, tmp_acc_outbuf);
		...
		//	g_sport_acc_cnt = 0;
		//}
		//g_sport_acc[g_sport_acc_cnt] = (short)tmp_acc_inbuf[6];
		//g_sport_acc_cnt = g_sport_acc_cnt + 1;
	...
	//stress_acc_get(accval);
	//Exercise_judge_acc_get(accval);
	//NRF_LOG_INFO("======acc_get_Exercise_flag(void) = %d\n",acc_get_Exercise_flag());
