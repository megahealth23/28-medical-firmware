...
uint8_t MotionState[MS_datalength];
...
void waveletProc(int *in, int inlen, int slevel, int elevel, int *out, float *PeakVect, int *Peakcnt)
{
...
void get_spo2(int *red, int *infra, int *green, int inlen, int *Acc, int Acclen, AlgProc_Para *AlgPara)
{
	...
	wavelet_baseProc(infra+Marglen, Waveletlen, 1, 8, wrcoefbuf_base);
	if (RING_AFE_HRV_ON_MODE == (RING_AFE_HRV_ON_MODE & afe_get_work_mode())) {
		waveletProc(green + Marglen, Waveletlen, slevel, elevel, wrcoefbuf, Owpeakvect, &peakcnt);
		//waveletProc(infra + Marglen, Waveletlen, slevel, elevel, wrcoefbuf, Owpeakvect, &peakcnt);
	} else {
		waveletProc(infra + Marglen, Waveletlen, slevel, elevel, wrcoefbuf, Owpeakvect, &peakcnt);
	}
...
//	printf("%f ", WaveDvrate);
	...
	peakcnt = 0;
	waveletProc(red + Marglen, Waveletlen, slevel, elevel, wrcoefbuf, Rwpeakvect, &peakcnt);
...
//off hand detect
    ...
    if(!(changecnt%(60/frame_length)))
    {
        ...
        int change_step = 1500/frame_length;
...
//            }
    ...
    {	
        ...
        if(g_afe.offhand_flg == 0) // on hand
        {
            if(g_afe.flag != SPO2_HR_VALID || (g_afe.flag == SPO2_HR_VALID && summotionstate == 0) )
            {
					if(redreg == 0)
					{
					...
					else if(tmp_Red_orig > 2e6 && redreg > 8)
					{
	...
	//change infra 			
					if(tmp_Infra_orig < 2.5e5 && tmp_Red_orig > infrathreslow && infrareg < 60)
					{
...
void ble_send_persecond(void)
{
    ...
    frame_buf[5] = g_afe.shr;//red_detec.offhand; //extend write
    frame_buf[6] = g_afe.hr; //extend write
    frame_buf[7] = g_afe.spo2; //extend write
	frame_buf[8] = g_afe.breathrate;
	frame_buf[9] = BR_vect[BR_cnt-1];
	frame_buf[10] = BR_cnt;
//	frame_buf[9] = (char)(test_accval >> 8);
//	frame_buf[10] = (char)(test_accval);    
//	frame_buf[14] = (char)(num_calculation >> 8);
//	frame_buf[15] = (char)(num_calculation);
...
//        static int datano = 0;
//        if(datano < MS_datalength - 1)
//        {
//            MotionState[datano] = accdata.movement;
//            datano++;
//        }
//        else
//        {
//            MotionState[MS_datalength - 1] = accdata.movement;
//            memcpy(MotionState, MotionState + 1, sizeof(uint8_t)*(MS_datalength - 1));
//        }
//        summotionstate = 0;
...
//        }
        ...
        {
            get_spo2(spo2_red, spo2_infra, spo2_green, length_wavelet, AccData, length_accval, &AlgPara);
...
//      AlgPara.HrCnt = 64; // Only for test.
		if(HRV_timestamp == 0)      //add first time stamp
		{
          ...
          //          HRV_vect[HRV_cnt] = (v++) % 256; // 0x01;// Only for test.
            ...
            if((AlgPara.Spo2 != 0 && AlgPara.SSpo2 != 0) && AlgPara.heartrate != 0)
            {
                g_afe.flag = SPO2_HR_VALID;
                spo2valflg = 1;
            }
...
void afe_wave_free(void)
{
	...
	fifo32_init(&spo2_red_fifo,length_rawdata, spo2_red);
	fifo32_init(&spo2_infra_fifo, length_rawdata, spo2_infra);
	fifo32_init(&spo2_green_fifo, length_rawdata, spo2_infra);
}
