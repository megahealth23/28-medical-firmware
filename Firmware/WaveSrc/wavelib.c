...
wave_object wave_init(char* wname) {
	...
	//90+4*4*8 = 218
	obj = (wave_object)nrf_malloc(sizeof(struct wave_set) + sizeof(float)* 4 * retval);
...
wt_object wt_init(wave_object wave,char* method, int siglength,int J) {
	...
	if (method == NULL) {
		obj = (wt_object)nrf_malloc(sizeof(struct wt_set) + sizeof(float)* (siglength +  2 * J * (size+1)));
		...
		obj = (wt_object)nrf_malloc(sizeof(struct wt_set) + sizeof(float)* (siglength * (J + 1)));
...
wtree_object wtree_init(wave_object wave, int siglength,int J) {
	...
	obj = (wtree_object)nrf_malloc(sizeof(struct wtree_set) + sizeof(float)* (siglength * (J + 1) + elength + nodes + J + 1));
...
wpt_object wpt_init(wave_object wave, int siglength, int J) {
	...
	obj = (wpt_object)nrf_malloc(sizeof(struct wpt_set) + sizeof(float)* (elength + 4 * nodes + 2 * J + 6));
...
cwt_object cwt_init(char* wave, float param,int siglength, float dt, int J) {
	...
	N = siglength;
	nj2 = 2 * N * J;
	obj = (cwt_object)nrf_malloc(sizeof(struct cwt_set) + sizeof(float)* (nj2 + 2 * J + N));
	...
	t1 = 0.499999 + log((float)N) / log(2.0);
	ibase2 = 1 + (int)t1;

	obj->npad = (int)pow(2.0, (float)ibase2);
...
static void dwt_per(wt_object wt, float *inp, int N, float *cA, int len_cA, float *cD, int len_cD) {
...
static void wtree_per(wtree_object wt, float *inp, int N, float *cA, int len_cA, float *cD, int len_cD) {
	int l, l2, isodd, i, t, len_avg;
...
static void dwpt_per(wpt_object wt, float *inp, int N, float *cA, int len_cA, float *cD, int len_cD) {
	int l, l2, isodd, i, t, len_avg;
...
static void wtree_sym(wtree_object wt, float *inp, int N, float *cA, int len_cA, float *cD, int len_cD) {
	int i, l, t, len_avg;
...
static void dwpt_sym(wpt_object wt, float *inp, int N, float *cA, int len_cA, float *cD, int len_cD) {
	int i, l, t, len_avg;
...
/*
static void dwt1(wt_object wt,int *sig,int len_sig, float *cA, int len_cA, float *cD, int len_cD) {
	int len_avg,D,lf;
	float *cA_undec;
	int*signal;
	len_avg = (wt->wave->lpd_len + wt->wave->hpd_len) / 2;
	//len_sig = 2 * (int)ceil((float)len_sig / 2.0);

	D = 2;

	if (!strcmp(wt->ext, "per")) {
		signal = (int*)malloc(sizeof(int)* (len_sig + len_avg + (len_sig % 2)));

		len_sig = per_ext(sig, len_sig, len_avg / 2, signal);

		cA_undec = (float*)malloc(sizeof(float)* (len_sig + len_avg + wt->wave->lpd_len - 1));
		...
		signal = (float*)malloc(sizeof(float)* (len_sig + 2 * (lf - 1)));

		len_sig = symm_ext(sig, len_sig, lf - 1, signal);

		cA_undec = (float*)malloc(sizeof(float)* (len_sig + 3 * (lf - 1)));
...
void dwt(wt_object wt,int *inp) {
	...
	wt->outlength = 0;
	wt->zpad = 0;
	orig = (float*)nrf_malloc(sizeof(float)* temp_len);//orig as input;1K*4
	orig2 = (float*)nrf_malloc(sizeof(float)* temp_len);//1K*4
...
// replace orig with inp
   ...
   if (!strcmp(wt->ext,"sym")) {
		...
		for (iter = 0; iter < J; ++iter) {
			...
			dwt_sym(wt, orig, temp_len, orig2, len_cA, wt->params + N, len_cA);// wt->params + N ³¤¶È£»
...
// replace orig with inp
   ...
   if (!strcmp(wt->ext,"sym")) {
		...
		for (iter = 0; iter < J; ++iter) {
			...
			dwt_sym(wt, orig, temp_len, orig2, len_cA, wt->params + N, len_cA);// wt->params + N ���ȣ�
...
void wtree(wtree_object wt,float *inp) {
	...
	wt->outlength = 0;
	wt->zpad = 0;
	orig = (float*)nrf_malloc(sizeof(float)* temp_len);
	/*
	if ((temp_len % 2) == 0) {
		wt->zpad = 0;
...
void dwpt(wpt_object wt, float *inp) {
	...
	eparam = wt->eparam;
	orig = (float*)nrf_malloc(sizeof(float)* temp_len);
	tree = (float*)nrf_malloc(sizeof(float)* (temp_len * (J + 1) + elength));
	nodelength = (int*)nrf_malloc(sizeof(int)* nodes);
...
int getCWTScaleLength(int N) {
	...
	temp = (log((float)N / 2.0) / log(2.0)) / dj;
	J = (int)temp;
...
void setCWTScales(cwt_object wt, float s0, float dj,char *type,int power) {
	int i;
	strcpy(wt->type,type);
	//s0*pow(2.0, (float)(j - 1)*dj);
	if (!strcmp(wt->type, "pow") || !strcmp(wt->type, "power")) {
		for (i = 0; i < wt->J; ++i) {
			wt->scale[i] = s0*pow((float) power, (float)(i)*dj);
		}
		wt->sflag = 1;
		wt->pow = power;
...
void cwt(cwt_object wt, float *inp) {
	...
	if (wt->sflag == 0) {
		for (i = 0; i < wt->J; ++i) {
			wt->scale[i] = wt->s0*pow(2.0, (float)(i)*wt->dj);
		}
		wt->sflag = 1;
	}
...
static void idwt1(wt_object wt,float *temp, float *cA_up,float *cA, int len_cA,float *cD,int len_cD,float *X_lp,float *X_hp,float *X) {
...
static void idwt_per(wt_object wt, float *cA, int len_cA, float *cD, int len_cD,  float *X) {
...
void idwt(wt_object wt, int *dwtop) {
	...
	int app_len, det_len;
	float *cA_up, *X_lp, *X_hp,*out,*temp;

	J = wt->J;
	U = 2;
	app_len = wt->length[0];
	out = (float*)nrf_malloc(sizeof(float)* (wt->siglength + 1));//4K
	
	if (!strcmp(wt->ext, "sym") && !strcmp(wt->cmethod, "direct")) {
		app_len = wt->length[0];
		det_len = wt->length[1];
		N = 2 * wt->length[J] - 1;//515*2=1029
		lf = (wt->wave->lpr_len + wt->wave->hpr_len) / 2;

		X_lp = (float*)nrf_malloc(sizeof(float)* (N + 2 * lf - 1));//4*(1029+7)=4k
...
static void idwpt_per(wpt_object wt, float *cA, int len_cA, float *cD, int len_cD, float *X) {
	int len_avg, i, l, m, n, t, l2;
...
static void idwpt_sym(wpt_object wt, float *cA, int len_cA, float *cD, int len_cD, float *X) {
	int len_avg, i, l, m, n, t, v;
...
void idwpt(wpt_object wt, float *dwtop) {
	...
	X_lp = (float*)nrf_malloc(sizeof(float)* 2 * (wt->length[J] + lf));
	X = (float*)nrf_malloc(sizeof(float)* xlen);
	out = (float*)nrf_malloc(sizeof(float)* wt->length[J]);
	out2 = (float*)nrf_malloc(sizeof(float)* wt->length[J]);
	prep = (int*)nrf_malloc(sizeof(int)* p);
	ptemp = (int*)nrf_malloc(sizeof(int)* p);
	n1 = 1;
	llb = 1;
	index2 = xlen / p;
	indexp = 0;
	if (wt->basisvector[0] == 1) {
		...
		if (!strcmp(wt->ext, "per")) {
			...
			//X_lp = (float*)nrf_malloc(sizeof(float)* (N + 2 * lf - 1));
			index = 0;
...
static void swt_per(wt_object wt,int M, float *inp, int N, float *cA, int len_cA, float *cD, int len_cD) {
...
static void swt_fft(wt_object wt, float *inp) {
	int i, J, temp_len, iter, M, N, len_filt;
	int lenacc;
	float *low_pass, *high_pass,*sig,*cA,*cD;
	...
	low_pass = (float*)nrf_malloc(sizeof(float)* M * len_filt);
	high_pass = (float*)nrf_malloc(sizeof(float)* M * len_filt);
	sig = (float*)nrf_malloc(sizeof(float)* (M * len_filt + temp_len + (temp_len%2)));
	cA = (float*)nrf_malloc(sizeof(float)* (2 * M * len_filt + temp_len + (temp_len % 2)) - 1);
	cD = (float*)nrf_malloc(sizeof(float)* (2 * M * len_filt + temp_len + (temp_len % 2)) - 1);
...
static void swt_direct(wt_object wt, float *inp) {
	...
	cA = (float*)nrf_malloc(sizeof(float)* temp_len);
	cD = (float*)nrf_malloc(sizeof(float)* temp_len);
...
void iswt(wt_object wt, float *swtop) {
	...
	float *appx1, *det1,*appx_sig,*det_sig,*cL0,*cH0,*tempx,*oup00L,*oup00H,*oup00,*oup01,*appx2,*det2;
	...
	appx_sig = (float*)nrf_malloc(sizeof(float)* N);
	det_sig = (float*)nrf_malloc(sizeof(float)* N);
	appx1 = (float*)nrf_malloc(sizeof(float)* N);
	det1 = (float*)nrf_malloc(sizeof(float)* N);
	appx2 = (float*)nrf_malloc(sizeof(float)* N);
	det2 = (float*)nrf_malloc(sizeof(float)* N);
	tempx = (float*)nrf_malloc(sizeof(float)* N);
	cL0 = (float*)nrf_malloc(sizeof(float)* (N + (N%2) + lf));
	cH0 = (float*)nrf_malloc(sizeof(float)* (N + (N % 2) + lf));
	oup00L = (float*)nrf_malloc(sizeof(float)* (N + 2 * lf));
	oup00H = (float*)nrf_malloc(sizeof(float)* (N + 2 * lf));
	oup00 = (float*)nrf_malloc(sizeof(float)* N);
	oup01 = (float*)nrf_malloc(sizeof(float)* N);
	...
	for (iter = 0; iter < J; ++iter) {
		...
		value = (int)pow(2.0, (float) (J - 1 - iter));
...
static void modwt_per(wt_object wt, int M, float *inp, int N, float *cA, int len_cA, float *cD, int len_cD) {
	int l, i, t, len_avg;
	float s;
	float *filt;
	len_avg = wt->wave->lpd_len;

	filt = (float*)nrf_malloc(sizeof(float)* 2 * len_avg);
	s = sqrt(2.0);
	for (i = 0; i < len_avg; ++i) {
...
static void imodwt_per(wt_object wt,int M, float *cA, int len_cA, float *cD, int len_cD, float *X) {
	int len_avg, i, l, t;
	float s;
	float *filt;
	len_avg = wt->wave->lpd_len;
...
void imodwt(wt_object wt, float *dwtop) {
	...
	lf = wt->wave->lpr_len;
	lenacc = N;
	M = (int)pow(2.0, (float)J - 1.0);
	//M = 1;
	X = (float*)nrf_malloc(sizeof(float)* N);
