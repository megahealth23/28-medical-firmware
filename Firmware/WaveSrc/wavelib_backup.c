...
/*
static void dwt1(wt_object wt,int *sig,int len_sig, double *cA, int len_cA, double *cD, int len_cD) {
	int len_avg,D,lf;
	double *cA_undec;
	int*signal;
...
void dwt(wt_object wt,int *inp) {
	...
	wt->outlength = 0;
	wt->zpad = 0;
	orig = (double*)malloc(sizeof(double)* temp_len);//orig as input;
...
static void swt_fft(wt_object wt, double *inp) {
	...
	int lenacc;
	double *low_pass, *high_pass,*cA,*cD,*sig;
