...
from afe_object import AFE_Object 

class EHR_Object(AFE_Object):
    def __init__(self):
...
# 4Byte:  the bytes of Notify Frame 
        ...
        self.RawDataTYPE_EHR = 96

 #  20201030，这里是第一次测试版本，先加2，进行测试
 #  正式版本更改为0，因为林总认为加2不妥
 #  20201228，这里是第二次测试版本，加1，仅附加帧的长度，并进行测试
 #  正式版留意邮件附件
        self.EHR_RAWDATA_OFFSET_V1 = self.RAWDATA_OFFSET_V1 + 1
        self.EHR_RAWDATA_OFFSET_V2 = self.RAWDATA_OFFSET_V2 + 1

        self.ELEMT_EHR_CNT = 0
        self.ELEMT_EHR_SIZE = 5
...
############  V2:the un-fixed length(greater than or equal: 6=2+4)   ##########
    ...
    def get_ehr_frames_v1(self, old_stream, new_stream):
        work_stream = []
        if old_stream:
        ...
        if  (self.PROTOCOL_V1_LENGTH > stream_length):
            ...
            elemt_cnt = 0
            ptt_frames = []
            return (elemt_cnt, ptt_frames, remaining_stream)
        ...
        return (elemt_cnt, ptt_frames, remaining_stream, index)
#***************************************************#    
    def get_ehr_frames_v2(self, old_stream, new_stream):
        work_stream = []
        ...
        return (elemt_cnt, ptt_frames, remaining_stream, index)     
#***************************************************#
    def extract_ehr_data(self, ptt_frames, elemt_cnt):
        result = True
        ecg_arr = []
        greenled_arr=[]
        ...
        print("elemt_cnt")
        print(elemt_cnt)
        print("extract_ptt_data ptt_frames")
        print(ptt_frames)

        if ((self.ELEMT_PTT_SIZE*elemt_cnt) != len(ptt_frames)):
            print("(self.ELEMT_PTT_SIZE*elemt_cnt) != len(ptt_frames)")
            result = False
            greenled_arr = []
            ecg_arr = []
        ...
        for ite in range(elemt_cnt):
            ...
            greenled_val = AFE_Object.afe_convert_sensor_data( (ptt_frames[frame_sidx]<<0 ), (ptt_frames[frame_sidx+1]<<0), 0)
            ecg_val = AFE_Object.afe_convert_sensor_data( (ptt_frames[frame_sidx+2]<<0), (ptt_frames[frame_sidx+3]<<0),  (ptt_frames[frame_sidx+4]<<0) )
