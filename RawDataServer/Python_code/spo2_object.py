...
# 4Byte:  the bytes of Notify Frame 
        ...
        self.RawDataTYPE_SPO2 = 96

 #  20201030，这里是第一次测试版本，先加2，进行测试
 #  正式版本更改为0，因为林总认为加2不妥
 #  20201228，这里是第二次测试版本，加1，仅附加帧的长度，并进行测试
 #  正式版留意邮件附件
        self.SPO2_RAWDATA_OFFSET_V1 = self.RAWDATA_OFFSET_V1 + 1
        self.SPO2_RAWDATA_OFFSET_V2 = self.RAWDATA_OFFSET_V2 + 1

        self.ELEMT_SPO2_CNT = 0
        self.ELEMT_SPO2_SIZE = 5
...
############  V2:the un-fixed length(greater than or equal: 6=2+4)   ##########
    ...
    def get_spo2_frames_v1(self, old_stream, new_stream):
        work_stream = []
        if old_stream:
        ...
        return (elemt_cnt, ptt_frames, remaining_stream, index)
#***************************************************#    
    def get_spo2_frames_v2(self, old_stream, new_stream):
        work_stream = []
...
#***************************************************#
    def extract_ptt_data(self, ptt_frames, elemt_cnt):
        ...
        for ite in range(elemt_cnt):
            ...
            greenled_val = afe.afe_convert_sensor_data( (ptt_frames[frame_sidx]<<0 ), (ptt_frames[frame_sidx+1]<<0), 0)
            ecg_val = afe.afe_convert_sensor_data( (ptt_frames[frame_sidx+2]<<0), (ptt_frames[frame_sidx+3]<<0),  (ptt_frames[frame_sidx+4]<<0) )
