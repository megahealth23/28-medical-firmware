% %  real time plot: if PTT modle, change the type to PTT, IF
% SPO2mode,change the mode to SPO2;
...
while 1
    ...
    if length(data) ~= 0 % not equal 读到1Byte;
        ...
        elseif TYPE== SPO2                
                topcheck=94;%SPO2
                [sid] = find(datatmp==topcheck);%%SPO2标记索引；
...
% %         CASE SPO2
        if (length(sid) ~= 0 && TYPE== SPO2)
            datatmp = datatmp(sid(1):end);%（1：end）
            if(length(datatmp) >= 244)%从5B开始有9个数；
                dnum = datatmp(1:244);
                datatmp = datatmp(245:end);%下一批数据，一包20Byte;
                if (dnum(1) == topcheck && dnum(2) == 1)
                    framecount = dnum(7);
                    tmpbuf = dnum(8:(8+framecount*9-1));
                    framcnt = 1;
                    while(framcnt < framecount*9)
                        ch1(datano) = afe_convert_sensor_data(tmpbuf(framcnt), tmpbuf(framcnt+1), tmpbuf(framcnt+2));
                        ch2(datano) = afe_convert_sensor_data(tmpbuf(framcnt+3), tmpbuf(framcnt+4), tmpbuf(framcnt+5));
                        ch3(datano) = afe_convert_sensor_data(tmpbuf(framcnt+6), tmpbuf(framcnt+7), tmpbuf(framcnt+8));
                        datano = datano + 1;
                        framcnt = framcnt + 9;
                        if mod(datano,5) == 0 %mod,取余；
                            pause(0.0001);
                            subplot(3,1,1);axis([1 200 0 4000]);plot(ch1(max(1,datano-200):end));
                            subplot(3,1,2);axis([1 200 0 4000]);plot(ch2(max(1,datano-200):end));
                            subplot(3,1,3);axis([1 200 0 4000]);plot(ch3(max(1,datano-200):end));
                        end
